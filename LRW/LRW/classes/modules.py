...
def attention(query, key, value, mask=None, dropout=None):
    ...
    with autocast(enabled=False):
        if mask is not None:
            adjusted_mask = adjust_mask(mask,query.size(1), query.size(2))
            scores = scores.float()
...
def adjust_mask(mask, num_heads, seq_len_query):
    if mask.dim() == 3:
        mask = mask.unsqueeze(1)  # Add a dimension for num_heads
        mask = mask.expand(-1, num_heads, seq_len_query, -1)  # Expand for all heads and query length
    return mask
...
class CNN_3d_featextractor(nn.Module):
    ...
    @autocast()
    def forward(self, faces, mask):
        # print("Input size:", faces.size())
        # print("faces shape", faces.shape)
        # print("mask shape", mask.shape)
        # exit()
        assert faces.size(3) == 96
        assert faces.size(-1) == 96
        faces = faces.permute(0,2,1,3,4)
...
class VTP(nn.Module):   
    def __init__(self, num_layers, dims, heads, patch_sizes, initial_resolution=48, 
        ...
        for l, dim, h, p in zip(num_layers, dims, heads, patch_sizes):
            ...
            # print(f"Layer: {l}, Patch Size: {p}, Current/Target/Calculated Dim: {cur_dim}/{dim}/{input_dim}")
            if input_dim == dim:
                # print("Dimension match: Adding simple Rearrange.")
                self.patch_projectors.append(Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', 
                                                        p1 = p, p2 = p))
            else:
                # print("Dimension mismatch: Adding Rearrange followed by MLP adjustment.")
                self.patch_projectors.append(nn.Sequential(
                                                 Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', 
                                                    p1 = p, p2 = p),
                                                 Mlp(input_dim, dim, dim)))
            # print(f"Added Layer with Patch Size: {p} and Head: {h}")
            cur_res /= p
    ...
    def forward(self, face_tokens, cls_token=None):
        ...
        for i, (patch_maker, transformer) in enumerate(zip(self.patch_projectors, 
            ...
            x = patch_maker(x)
            # print("Input x shape after patch projection:", x.shape)
            if i == len(self.patch_projectors) - 1 and cls_token is not None:
                x = torch.cat([cls_token, x], dim=1)
                # print("Input x shape after concat:", x.shape)
            ...
            if i != len(self.patch_projectors) - 1:
                ...
                # print("Input x shape after feature map projector:", x.shape)
