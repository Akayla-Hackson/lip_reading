...
class PositionalEncoding(nn.Module):
  def __init__(self, d_model: int, dropout=0.0,  max_length: int = 5000):
      # d_model:      dimension of embeddings
      # dropout:      randomly zeroes-out some of the inpt
...
class Transformer(nn.Module):
    def __init__(self, feature_size=256, num_heads=8, num_layers=6):
        super().__init__()
        ...
        self.embedding = nn.Embedding(self.vocab_size, feature_size, padding_idx=self.pad_token_id)
        transformer_decoder_layer = nn.TransformerDecoderLayer(d_model=feature_size, nhead=num_heads)
        self.transformer_decoder = nn.TransformerDecoder(transformer_decoder_layer, num_layers=num_layers)
        self.output_layer = nn.Linear(feature_size, self.vocab_size)
    ...
    def forward(self, x, tgt, max_len=100, train=False):
        if not train:
            device = x.device
            outputs = torch.LongTensor([[self.start_token_id]]).to(device)
            
            # Initialize storage for logits with a fixed size, [max_len, 1, vocab_size]
            logits_history = torch.zeros(max_len, 1, self.vocab_size, device=device)
            ...
            for i in range(max_len):
                ...
                logits = self.output_layer(out[-1, :, :].unsqueeze(0)) # Process the last output only
                ...
                if next_token.item() == self.stop_token_id:
                    print(f"Stop token ({self.stop_token_id}) predicted at step {i}")
                    break
